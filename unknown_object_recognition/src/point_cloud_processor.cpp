...
private:
  ...
  using PEPCL = happymimi_recognition_msgs::PositionEstimatorPCL;
  ...
  int cls_three_min_size, cls_three_max_size, cls_inlier_min_size, cls_inlier_max_size;
  std::string filter_inlier_axis="x";
  std::map<std::string, std::vector<bool>> pcp_config;

public:
  PointCloudProcessor() : nh("~") {
    ...
    horizon_ss_ = nh.advertiseService("/uor/pcp/horizon_centroids", &PointCloudProcessor::horizonRecSCB, this);
    ...
    // }
    bool distance_thres_flg, downsample_flg, pppc_flg, horizon_flg, horizon_centroids_flg, table_plane_flg, table_object_cluster_flg, foreground_centroid_flg;
